...
export function CommunicationsPanel() {
  ...
  return <div className="communications-workspace">
    ...
    <section className="panel-card preview-panel"><header><p className="eyebrow">Live preview</p><h2>{template?.subject.replaceAll("{proposal_title}", "Taming 40-Minute CI").replaceAll("{session_title}", "Taming 40-Minute CI")}</h2></header>
      <div className="email-preview"><div><span className="avatar">PR</span><span><strong>Program Desk</strong><small>to Priya Raman</small></span></div><p>{String(template?.body ?? "").replaceAll("{speaker_name}", "Priya").replaceAll("{proposal_title}", "Taming 40-Minute CI").replaceAll("{session_title}", "Taming 40-Minute CI").replaceAll("{assignment_count}", "4").replaceAll("{deadline}", "October 15")}</p></div>
    </section>
